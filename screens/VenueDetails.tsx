...
const VenueDetails: React.FC = () => {
  ...
  const getAmenityIcon = (amenityName: string) => {
    const iconProps = { className: "w-6 h-6 text-accent shrink-0" };
  ...
  if (error || !venue) {
    return <div className="text-center py-20 text-red-600 bg-primary">{error || 'Venue not found.'}</div>;
  }
    ...
    <div className="bg-primary">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* Left/Main Column */}
          <div className="lg:col-span-2 bg-surface p-8 rounded-2xl shadow-xl-soft border border-border">
             {/* Header */}
            <div className="mb-6">
              <h1 className="text-3xl md:text-4xl font-bold text-secondary">{venue.name}</h1>
              <div className="flex flex-wrap items-center mt-3 text-secondary/80 gap-x-4 gap-y-2">
                ...
                <div className="flex items-center">
                  ...
                  <span className="ml-2 text-sm font-semibold">{venue.rating.toFixed(1)} ({venue.reviewCount} reviews)</span>
                </div>
              </div>
            </div>
                        ...
                        <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
                            {venue.images.slice(0, 5).map((img, index) => (
                                <button
                                    ...
                                    className={`block rounded-lg overflow-hidden focus:outline-none transition-all duration-200 ${selectedImage === img ? 'ring-4 ring-accent ring-offset-2' : 'hover:opacity-80'}`}
                                >
                                    <img 
                                        src={img} 
            ...
            <div className="space-y-8">
                {venue.about && (
                    <div className="border-b border-border pb-6">
                        <h2 className="text-2xl font-bold text-secondary mb-4">About this venue</h2>
                        <p className="text-secondary/80 leading-relaxed whitespace-pre-wrap">{venue.about}</p>
                    </div>
                )}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-5">
                        {venue.amenities.map(amenity => (
                            <div key={amenity} className="flex items-center">
                                ...
                                <span className="ml-3 text-secondary font-medium">{amenity}</span>
                            </div>
                        ))}
                    </div>
                </div>
               ...
               <div>
                <h2 className="text-2xl font-bold text-secondary mb-4">Sports Available</h2>
                 <div className="flex flex-wrap gap-2">
                  {venue.sports.length > 0 ? venue.sports.map(sport => (
                    <div key={sport} className="bg-accent/10 text-accent font-semibold px-4 py-1.5 rounded-full text-sm capitalize">
                      {sport}
                    </div>
                  )) : (
                    <p className="text-secondary/70">No specific sports listed for this venue.</p>
                  )}
                </div>
              </div>
          ...
          <div className="lg:col-span-1">
             <div className="sticky top-24 space-y-6">
                <div className="bg-surface border border-border rounded-xl p-6 text-center shadow-xl-soft">
                   {venue.pricePerHour && (
                        <div className="mb-4">
                            <p className="text-sm text-secondary/70">Starts from</p>
                            <p className="text-4xl font-bold text-accent">₹{venue.pricePerHour}<span className="text-lg font-normal text-secondary/70">/hr</span></p>
                        </div>
                   )}
                   <p className="text-secondary/80 mb-4">Ready to play? Book your slot now!</p>
                   <button 
                    onClick={() => navigate(`/book/${venue.id}`)}
                    className="w-full bg-accent text-white font-bold py-3 rounded-lg hover:bg-accent-dark transition-all transform hover:scale-105 duration-300 shadow-lg-accent">
                        Book This Venue
                    </button>
                </div>

                <div className="bg-surface border border-border rounded-xl p-6 shadow-xl-soft">
                    <h3 className="font-bold text-lg text-secondary mb-4">Venue Information</h3>
                     <div className="space-y-4 text-sm">
                        <div className="flex items-start">
                            <ClockIcon className="w-5 h-5 text-accent mt-0.5 mr-3 shrink-0" />
                            <div>
                                <span className="font-semibold text-secondary">Hours: </span>
                                {venue.operatingHours ? (
                                    <span className="text-secondary/80">{venue.operatingHours}</span>
                                ) : (
                                    <span className="text-secondary/60 italic">Not available</span>
                                )}
                            </div>
                        </div>
                        <div className="flex items-start">
                            <PhoneIcon className="w-5 h-5 text-accent mt-0.5 mr-3 shrink-0" />
                             <div>
                                <span className="font-semibold text-secondary">Phone: </span>
                                {venue.phone ? (
                                    <a href={`tel:${venue.phone}`} className="text-accent hover:underline">{venue.phone}</a>
                                ) : (
                                    <span className="text-secondary/60 italic">Not available</span>
                                )}
                            </div>
                        ...
                        <div className="flex items-start">
                            <GlobeAltIcon className="w-5 h-5 text-accent mt-0.5 mr-3 shrink-0" />
                            <div>
                                <span className="font-semibold text-secondary">Website: </span>
                                 {venue.website ? (
                                    <a href={venue.website} target="_blank" rel="noopener noreferrer" className="text-accent hover:underline break-all">{venue.website}</a>
                                 ): (
                                    <span className="text-secondary/60 italic">Not available</span>
                                 )}
                            </div>
                ...
                <div className="bg-surface border border-border rounded-xl p-6 shadow-xl-soft">
                    <h3 className="font-bold text-lg text-secondary mb-4">Location & Directions</h3>
                    <div className="w-full h-60 bg-gray-300 rounded-lg overflow-hidden mb-4">
                     ...
                     <button
                        ...
                        className="w-full flex items-center justify-center gap-2 bg-secondary text-white font-semibold py-3 rounded-lg hover:bg-secondary/80 transition-all disabled:opacity-50">
        ...
        <div className="mt-12 pt-8 border-t border-border">
          <h2 className="text-2xl font-bold text-secondary mb-6">Player Reviews (Sample)</h2>
          <div className="space-y-6">
            {REVIEWS.map(review => (
              <div key={review.id} className="bg-surface p-5 rounded-lg border border-border/70 shadow-lg-soft">
                <div className="flex items-start">
                  <img src={review.user.avatar} alt={review.user.fullName} className="w-12 h-12 rounded-full mr-4" />
                  <div className="flex-1">
                    <div className="flex justify-between items-center flex-wrap">
                      <div>
                        <p className="font-semibold text-secondary">{review.user.fullName}</p>
                        <StarRating rating={review.rating} />
                      </div>
                      <p className="text-sm text-secondary/60 mt-1 sm:mt-0">{review.date}</p>
                    </div>
                    <p className="mt-2 text-secondary/80">{review.comment}</p>
                  </div>
                </div>
              </div>
            ))}
