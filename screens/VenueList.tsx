    ...
    return (
        <div className="relative">
            <input 
                ...
                className="w-full p-4 pl-12 border border-gray-300 rounded-full shadow-sm focus:ring-2 focus:ring-primary focus:border-primary transition" />
                ...
                <ul className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                    {suggestions.map((s) => (
                        <li key={s.properties.place_id} 
                            ...
                            className="p-3 hover:bg-primary-light cursor-pointer border-b last:border-b-0">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 sticky top-24">
            ...
            <div className="mb-6">
                ...
                <button 
                    ...
                    disabled={isLocating}
                    className="w-full flex items-center justify-center bg-secondary text-text-dark font-semibold py-2.5 px-4 rounded-lg hover:bg-opacity-80 transition-all disabled:opacity-50 disabled:cursor-wait">
                    <LocationPinIcon className="w-5 h-5 mr-2" />
                    {isLocating ? 'Locating...' : 'Use My Current Location'}
                </button>
                 <p className="text-xs text-gray-500 mt-2 text-center">Searches for venues within a 10km radius.</p>
            </div>

            <div className="mb-6">
                <h4 className="font-semibold mb-3">Venue Type</h4>
                    ...
                    <legend className="sr-only">Venue Type</legend>
                     {(['all', 'indoor', 'outdoor'] as const).map(type => (
                         <div key={type} className="flex items-center">
                            ...
                            />
                            <label htmlFor={`type-${type}`} className="ml-3 text-sm text-gray-700 cursor-pointer capitalize">{type}</label>
                        </div>
                    ))}
                </fieldset>
            </div>
            ...
            <div className="mb-6">
                <h4 className="font-semibold mb-3">Sport Category</h4>
                <div className="space-y-2 max-h-60 overflow-y-auto pr-2">
                    {Object.entries(SPORT_CATEGORIES).map(([name, code]) => (
                        <div key={code} className="flex items-center">
                            ...
                            />
                            <label htmlFor={`cat-${code}`} className="ml-3 text-sm text-gray-700 cursor-pointer">{name}</label>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="mt-8">
                <button onClick={onApplyFilters} className="w-full bg-primary text-white py-2.5 rounded-lg font-bold hover:bg-opacity-90 transition-colors">Apply Filters</button>
                <button onClick={onClearFilters} className="w-full mt-2 text-gray-600 py-2 rounded-md hover:bg-gray-100 transition-colors">Clear Filters</button>
            </div>
        </div>
    </div>
);
        ...
        <div className="bg-gray-50">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div className="text-center mb-12">
                    <h1 className="text-4xl font-extrabold text-gray-800">Find & Book Sports Venues</h1>
                    <p className="text-gray-600 mt-2 max-w-2xl mx-auto">Discover real-world stadiums, courts, and sports centers near you. Use the filters to find your perfect spot.</p>
                    ...
                    <div className="w-full lg:w-3/4 lg:pl-4 mt-8 lg:mt-0">
                        {loading ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                             ...
                             <div className="text-center py-10 px-4 bg-yellow-50 text-yellow-800 rounded-lg">
                                <h3 className="font-bold">No Venues Found</h3>
