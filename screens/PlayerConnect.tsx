...
const ChatMessageBubble: React.FC<{ message: ChatMessage; currentUser: User | null }> = ({ message, currentUser }) => {
    ...
    const bubbleClasses = isMe ? 'bg-primary text-white' : 'bg-gray-200 text-text-dark';
    ...
    return (
        <div className={`flex items-end gap-3 ${containerClasses}`}>
            ...
            <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                {!isMe && <p className="text-xs text-gray-500 mb-1 ml-3">{message.user.fullName}</p>}
...
const PlayerConnect: React.FC = () => {
    ...
    const commonInputClass = "w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary bg-white";

    return (
        <div className="bg-gray-100">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
                 <div className="text-center mb-12">
                    <h1 className="text-4xl font-extrabold text-gray-800">Player Connect</h1>
                    <p className="text-gray-600 mt-2 max-w-2xl mx-auto">Find fellow players for a game! Post a message or use our AI assistant to help you write one.</p>
                </div>

                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    {/* Chat Section */}
                    <div className="lg:col-span-2 bg-white rounded-2xl shadow-lg flex flex-col h-[75vh]">
                        <div className="p-4 border-b font-bold text-lg text-text-dark">Community Chat</div>
                        ...
                        <div className="p-4 border-t bg-gray-50 rounded-b-2xl">
                            <form onSubmit={handleSendMessage} className="flex items-center gap-3">
                                <input
                                    ...
                                    className="w-full p-3 border border-gray-300 rounded-full focus:ring-2 focus:ring-primary focus:border-primary transition"
                                />
                                <button type="submit" className="bg-primary text-white font-semibold rounded-full p-3 hover:bg-opacity-90 transition-transform transform hover:scale-105">
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-white rounded-2xl shadow-lg sticky top-24">
                            <div className="p-4 border-b flex items-center gap-2">
                                ...
                                <h3 className="font-bold text-lg text-text-dark">AI Message Assistant</h3>
                            </div>
                            <form onSubmit={handleGenerateMessage} className="p-6 space-y-4">
                                ...
                                <button type="submit" disabled={isGenerating} className="w-full flex justify-center items-center gap-2 bg-accent text-white font-bold py-2.5 rounded-lg hover:bg-opacity-90 transition disabled:bg-opacity-50">
                                ...
                                <div className="px-6 pb-6">
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Generated Message</label>
                                     <textarea
                                        ...
                                        className={`w-full p-2 border rounded-md text-sm ${error ? 'border-red-300 bg-red-50 text-red-700' : 'border-gray-300 bg-gray-50'}`}
                                        rows={4}
                                    />
                                    {generatedMessage && !error && (
                                         <button onClick={(e) => handleSendMessage(e, generatedMessage)} className="w-full mt-2 bg-primary text-white font-semibold py-2 rounded-lg hover:bg-opacity-90 transition">
                                            Post to Chat
                                        </button>
                                    )}
                                </div>
