...
import { MOCK_USER, MOCK_FACILITATOR, MOCK_ADMIN } from '../constants';
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const adminLogin = async (email: string, pass: string): Promise<void> => {
    setLoading(true);
    return new Promise((resolve) => {
        setTimeout(() => {
            if (email === MOCK_ADMIN.email && pass === 'password') {
                const loggedInUser = MOCK_ADMIN;
                setUser(loggedInUser);
                localStorage.setItem('quickcourt_user', JSON.stringify(loggedInUser));
            }
            setLoading(false);
  ...
  const forgotPassword = async (): Promise<void> => {
    ...
    return new Promise(resolve => {
        setTimeout(() => {
            setLoading(false);
            // In a real app, this would trigger a backend API call to send a reset email
            resolve();
        }, 1000);
    });
  };
